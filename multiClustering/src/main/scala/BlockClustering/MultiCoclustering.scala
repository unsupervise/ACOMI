...
import breeze.stats.distributions.{Gamma, MultivariateGaussian, RandBasis, ThreadLocalRandomGenerator}
...
//  implicit val randBasis: RandBasis = new RandBasis(new ThreadLocalRandomGenerator(new MersenneTwister(54321)))
  ...
  def run(nIter: Int = 10,
          ...
          printLikelihood: Boolean = false): (List[Int], List[List[Int]]) = {
    ...
    (redundantColPartition, rowPartitions.toList)
